...
> **O login com Google no Android exige um development build.** No Expo Go o `expo-auth-session`
> gera o redirect `exp://127.0.0.1:8081`, e o Google recusa esse esquema logo na requisição de
> autorização, com `Error 400: invalid_request` ("Access blocked: Authorization Error"). Um OAuth
> client do tipo Android só aceita o esquema reverso do package
> (`com.fiap.cpmobile.chat:/oauthredirect`), que o Expo Go não pode usar por não ser esse app.
